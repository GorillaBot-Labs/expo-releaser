    ...
    it("updates an eas.json", async () => {
        ...
        await assertEasJson({
            build: {
                ...
                production: { releaseChannel: "production-1.1.0" }
            },
        })
        await assertSuccess()
    })
    ...
    it("throws an error for an invalid semver version", async () => {
        ...
        await assertEasJson({
            build: {
                ...
                production: { releaseChannel: "production-1.0.0" }
            },
        })
...
describe("-i 'ignore release channel versions'", () => {
    it("updates an app.config.js", async () => {
        const appConfigPath = await useFixture("release-branches/app.config.js")

        const args = {
            release: "1.1.0",
            ignoreReleaseChannel: true,
            appConfigPath,
        }
        await createReleaseCmd(args)
    ...
    it("updates an eas.json", async () => {
        const appConfigPath = await useFixture("release-branches/app.config.js")
        const easJsonPath = await useFixture("release-branches/eas.json")
    ...
    it("updates a package.json", async () => {
        const appConfigPath = await useFixture("release-branches/app.config.js")
        const packageJsonPath = await useFixture("release-branches/package.json")
